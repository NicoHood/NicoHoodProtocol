...
LedDevice * LedDeviceFactory::construct(const Json::Value & deviceConfig)
{
	...
	else if (type == "sharedmemory")
	{
		...
		const int sharedmemorykey = deviceConfig.get("shkey", 1213).asInt();
		device=  new LedDeviceMemory(sharedmemorykey);
	}

	else if (type == "serial")
	{
		// Get the parameters for the Serial device + baud
		const char *serialDevice = deviceConfig["output"].asCString();
		const unsigned long serialBaud   = deviceConfig["rate"].asUInt64();

		LedDeviceSerial* deviceSerial = new LedDeviceSerial();
